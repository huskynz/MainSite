...
namespace _12.Controllers
{
    public class HomeController : Controller
    {
        ...
        public HomeController(DiscordNotificationService discordNotificationService, TurnstileService turnstileService)
        {
        ...
        public async Task<IActionResult> Contact(string name, string email, string message)
        {
            ...
            var remoteIp = Request.Headers["CF-Connecting-IP"].FirstOrDefault()
                ?? HttpContext.Connection.RemoteIpAddress?.ToString();

            if (!await _turnstileService.VerifyAsync(turnstileToken, remoteIp))
            {
                TempData["ContactMessage"] = "We couldn't verify that you're human. Please try the challenge again.";
